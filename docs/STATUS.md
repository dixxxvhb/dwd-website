...
## Open
- 2026-09-12 — SCHEDULE + DROP-IN CART brief written: docs/plans/2026-09-12-schedule-and-drop-in-cart.md (new `/schedule/` route fed by the app's `public_site_schedule` RPC, cart → `drop-in-checkout` edge fn → Stripe, thank-you via `drop_in_order_public`). Audited 2026-09-12 with the app spec (feed sends `occurrence_date`, shows `date`; 35-minute hold). Nothing built. Blocked on the app spec's Phase 1 (dwd/docs/plans/2026-09-12-drop-in-web-checkout.md). Placement (own nav page) and the refund line locked with Dixon 2026-09-12; he still approves the rest of §4's copy before push.
