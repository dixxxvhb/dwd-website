...
## Done
- 2026-09-25 5590683 — AI FINDABILITY LIVE: Course JSON-LD for the 5 drop-ins, 3 ProSeries tracks and the Collective, #studio Place with real coordinates, /#hash URLs fixed; /llms.txt; IndexNow key + first Bing submission (HTTP 202, 8 URLs). Guards: scripts/check-classes-jsonld.mjs (drift vs live schedule), scripts/indexnow.mjs (re-ping after deploys). Why: ChatGPT referrals tap through at 34%.
