...
Date: 2026-09-12 · Repo: C:/Users/bowle/Code/dwd-website · Branch: `schedule-drop-ins` off main · Author: Fable · Status: BRIEF, waits on Phase 1 of the app spec
...
## 2. Files you own
- `index.html` — new `<section class="page" id="page-schedule" data-arm="ps">`, the nav item `Schedule` between ProSeries and Collective (desktop nav and the mobile menu), and one link inside each of the three `Weekly Schedule` track blocks: `This week's schedule and drop-ins →`.
- `js/schedule.js` — new: feed fetch, week switcher, cart, checkout POST, thank-you.
- `js/main.js` — only the route table entry for `schedule` and the script gate, if routes are registered there; nothing else.
